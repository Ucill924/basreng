...
def approve_token(w3, private_key, token_address, gas_price_gwei, chain_id, spender):
    try:
        ...
        print(
            Fore.GREEN + f"✅ Approve 1 {token_address[-6:]} sukses | TX: {w3.to_hex(tx_hash)}")
        time.sleep(2)
...
def get_price_back(token, address, input_token_address, amount, proxy=None):
    amount_wei = str(Web3.to_wei(amount, 'ether'))
    print(Fore.BLUE + f"🔁 Kembali swap {amount} token ke MONAD")
...
def report_trade_history(token, tx_hash, account, input_token_address, input_amount, output_token_address, output_amount, chain_id, proxy=None):
    ...
    try:
        ...
        if res.ok:
            ...
            print(Fore.CYAN + f"📝 Response: {res.json()}")
            print()
        else:
            print(Fore.RED + f"❌ Gagal report trade history! Status: {res.status_code}")
...
if __name__ == "__main__":
    ...
    for i, (address, pk) in enumerate(wallets, 1):
        try:
            ...
            if choice == "1":
                print(
                    Fore.CYAN + f"🔃 Mengirim approve TX untuk wallet {mask_address(address)}...")
                for name, token_address in TOKENS.items():
                    gwei = random.randint(
                        config["gwei_input_min"], config["gwei_input_max"])
                    print(Fore.YELLOW + f"  ➤ Approving token {name}...")
            ...
            if not signature:
                print(
                    Fore.RED + f"❌ Wallet [{i}] gagal melakukan sign message.")
                continue
            ...
            if not token:
                print(
                    Fore.RED + f"❌ Wallet [{i}] gagal mendapatkan token auth.")
                continue
            ...
            for cycle in range(1, swap_count + 1):
                ...
                swap_data = get_price(
                    token, address, proxy, output_token_address)
                if not swap_data:
                ...
                report_trade_history(
                    ...
                    output_amount=99999999,
                    chain_id=config["chainId"],
                    proxy=proxy
                )

                balance, decimals = check_token_balance(
                    w3, address, output_token_address)
                if not balance or balance == 0:
                    print(Fore.YELLOW + "⚠️ Tidak ada saldo untuk swap balik.")
                    continue

                print(
                    Fore.BLUE + f"🔁 Swap balik: seluruh saldo {balance:.6f} {output_token_name}")
                amount_wei = int(balance * (10 ** decimals))
                back_swap = get_price_back(
                    token, address, output_token_address, amount_wei, proxy)
                ...
                if tx_hash_back:
                    ...
                    print(Fore.BLUE +
                          f"💰 Saldo akhir MONAD: {final_balance:.6f}")
                else:
